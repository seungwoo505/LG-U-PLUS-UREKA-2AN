...
const names2: readonly string[] = ["lsw"];
 ...
 * 미리 할당하는 것을 추천
 */
function getTime2(a:number, b:number = 10) : number {
    return a * b;
}
...
function divide2({divisor} : {divisor : number}){
    return divisor;
}
...
/**
 * 화살표 함수의 결과 값을 미리 파악할 수 있다.
 * input 타입과 return 타입을 명시하면된다.
 * 화살표함수를 사용할 때 처음 함수명에 타입을 지정하면
 * 어차피 미리 작성된 화살표 함수에 맞춰서 작성해야하기 때문에 타입을 명시안해도 자동으로 지정된다.(input과 return 값만 해당)
 */
...
class Person{
    /**
     * public : 누구나 접근 가능
     * private : 할당한 클래스 내부에서만 접근이 가능하다.
     * protected : 상속받는 클래스에서 접근이 가능하다.
     * 
     * 아래의 변수같은 경우는 private로 두는 것이 일반적이다.
     * 개발자가 원하는 값이 있는데 public으로 둘 경우 무작위로 변경이 가능하기에
     * 기대하는 값이 안나올 수도 있어서 제어하기위해 private로 제어
     * 
     * 변경을 막고 싶으면 readonly를 하여 막을 수도 있다.(생성자로 생성할 때 한번만 작성되고 그다음 변경 불가)
     */
    private name : string;
    age : number;
